...
resource "aws_security_group" "redshift_sg" {
  ...
  ingress {
    ...
    to_port         = 5439
    protocol        = "tcp"
    security_groups = [aws_security_group.app_sg.id] # Allow access from app security group
  }
}
...
resource "aws_redshift_cluster" "default" {
  ...
  master_password        = var.redshift_master_password        # Handle securely (Vault or Secrets Manager)
  node_type              = "dc2.large"                         # Choose appropriate node type
  cluster_type           = "single-node"                       # Or multi-node if needed
  vpc_security_group_ids = [aws_security_group.redshift_sg.id] # Assign security group to Redshift cluster
}
